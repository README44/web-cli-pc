...
/**
 * @description 获取右上角铃铛的数据，并存在store里
 * @param {store} noticeStore notice的store
 * @returns {Array}
 * */
export async function getNoticeList(noticeStore: any) {
  const res = await api.notice.getNoticeList({ isRead: false });
  noticeStore.setNoticeList(res?.data?.list?.slice(0, 4) || []);
}

/**
 * @description 把所有router组装成新数据
 * @param {Array} routerList 路由
 * @param {Object} result 处理后的结果
 * @returns {Object}
 */
export const getAllQueryList = (routerList: any[], result: any = {}) => {
